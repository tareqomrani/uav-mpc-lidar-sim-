# app.py — Multi-Agent LiDAR + MPC (Maximal Build, Corrected)
# Features:
# - Timed dynamics (Δt, horizon H)
# - MPC with damped hover at goal (Option B)
# - Wind & gusts
# - Moving obstacles (bouncing)
# - LiDAR ray-casting + fused global hit map
# - Coverage grid (% area seen)
# - Battery/RTB energy model
# - Greedy task allocation (nearest goal)
# - Scenario presets (Warehouse / Urban Canyon / Disaster Mapping)
# - Metrics: distances, speeds, battery, min separation, latency
# - AI Suggestions: coverage, latency, safety, battery, speed
# - Exports: CSVs, JSON summary, PNG, GeoJSON, and one-click ZIP
...
class UAV:
        ...
        for _ in range(H):
            ...
            for ob in obstacles:
                ...
                if dist < 0.5:
                    ...
                    cost += 2.0 / (dist + 1e-3)
            # other drones (current positions as approximation)
            for od in others:
                if od.id == self.id:
                    continue
    ...
    def mpc_step(self, drones, obstacles, H=3, lidar_radius=6.0, damping=0.6, wind=np.zeros(2)):
        ...
        # damped hover within goal radius
        ...
        # MPC: discrete acceleration candidates
        acc = self.max_accel
    ...
    def _log_state(self):
        ...
        # toy power model: hover overhead + speed^3
...
# Greedy task allocation (assign nearest unassigned goal to each start)
assigned = set()
assigned_goals = []
for s in starts:
...
ax.set_title("LiDAR + MPC — Maximal Lab")
...
for d in drones:
    ...
    st.write(
        ...
        f"Speed(now) = {d.speed_hist[-1]:.2f} | Avg = {avg_spd:.2f} | Peak = {peak_spd:.2f} | "
...
high_batts = [m for m in metrics_list if m["battery_pct"] > 80.0]
if len(low_batts) > 0:
    sugs.append("One or more drones are below 20% battery. Trigger RTB or reduce mission duration.")
elif len(high_batts) == len(metrics_list) and len(metrics_list) > 0:
...
run_summary = {
    ...
    "obstacles": {"count": num_obstacles, "size": ob_size, "speed": ob_speed},
    "agents": {"num_drones": num_drones},
    "timing": {"steps": steps, "dt": dt, "total_time_s": sim_time, "horizon": horizon},
